...
export function GlobalSearch({ className }: GlobalSearchProps) {
  ...
  const handleAsk = async () => {
    ...
    const guidesContext = guides.map(g => `- ${g.title} (${g.agency_name}) [Guide ID: ${g.guide_id}]`).join('\n');

    const directoryContext = govDirectory.map(cat => {
      const sites = cat.links.map(l => `- ${l.name} → ${l.url} [/directory/${getSiteSlug(l.url)}]`).join('\n');
      return `Category: ${cat.name}\n${sites}`;
    }).join('\n\n');

    const context = `=== SERVICE GUIDES ===\n${guidesContext}\n\n=== GOVERNMENT DIRECTORY ===\n${directoryContext}`;
