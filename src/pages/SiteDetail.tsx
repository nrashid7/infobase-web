...
import { ExternalLink, Phone, Mail, MapPin, Clock, Globe, Loader2, AlertCircle, Building2, Target, ListChecks, Link2, ArrowLeft, FileText, Users, CreditCard, FileCheck, Calendar, Share2 } from 'lucide-react';
...
export default function SiteDetail() {
  ...
  const isScraped = siteDetails?.scrape_status === 'success' || siteDetails?.scrape_status === 'incomplete';
  const hasDetails = isScraped && (!!siteDetails?.description || (services && services.length > 0));
        ...
        {isLoading ? (
          ...
          <Card className="text-center py-16 border-dashed">
            <CardContent>
              ...
              <h3 className="text-xl font-semibold mb-3">
                {isScraped
                  ? (language === 'bn' ? 'সীমিত তথ্য উপলব্ধ' : 'Limited information available')
                  : (language === 'bn' ? 'বিস্তারিত তথ্য উপলব্ধ নেই' : 'No detailed information available yet')}
              </h3>
              <p className="text-muted-foreground max-w-md mx-auto">
                {isScraped
                  ? (language === 'bn'
                    ? 'এই সাইটের স্ক্র্যাপিং সীমিত ফলাফল দিয়েছে। অফিসিয়াল ওয়েবসাইট দেখুন।'
                    : 'Scraping returned minimal results for this site. Please visit the official website for complete information.')
                  : (language === 'bn' 
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            ...
            <div className="space-y-6">
              ...
              {/* Establishment Year */}
              {siteDetails.establishment_year && (
                <Card>
                  <CardContent className="pt-6">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-xs text-muted-foreground mb-0.5">{language === 'bn' ? 'প্রতিষ্ঠাকাল' : 'Established'}</p>
                        <p className="font-medium text-sm">{siteDetails.establishment_year}</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
              ...
              {/* Social Media */}
              {siteDetails.social_media && Object.values(siteDetails.social_media).some(Boolean) && (
                <Card>
                  <CardHeader className="pb-3">
                    <CardTitle className="flex items-center gap-2 text-lg">
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      {siteDetails.social_media.facebook && (
                        <a href={siteDetails.social_media.facebook} target="_blank" rel="noopener noreferrer"
                          className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-muted transition-colors text-sm">
                          <span className="w-5 h-5 flex items-center justify-center font-bold" style={{ color: '#1877F2' }}>f</span>
                          <span className="truncate flex-1">Facebook</span>
                          <ExternalLink className="w-3.5 h-3.5 text-muted-foreground" />
                        </a>
                      )}
                      {siteDetails.social_media.twitter && (
                        <a href={siteDetails.social_media.twitter} target="_blank" rel="noopener noreferrer"
                          className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-muted transition-colors text-sm">
                          <span className="w-5 h-5 flex items-center justify-center font-bold" style={{ color: '#1DA1F2' }}>𝕏</span>
                          <span className="truncate flex-1">Twitter / X</span>
                          <ExternalLink className="w-3.5 h-3.5 text-muted-foreground" />
                        </a>
                      )}
                      {siteDetails.social_media.youtube && (
                        <a href={siteDetails.social_media.youtube} target="_blank" rel="noopener noreferrer"
                          className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-muted transition-colors text-sm">
                          <span className="w-5 h-5 flex items-center justify-center font-bold" style={{ color: '#FF0000' }}>▶</span>
                          <span className="truncate flex-1">YouTube</span>
                          <ExternalLink className="w-3.5 h-3.5 text-muted-foreground" />
                        </a>
                      )}
                    </div>
