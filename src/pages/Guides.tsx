          ...
          {guides.map((guide, idx) => {
            ...
            return (
              <Link
                ...
                className={`modern-card p-5 group animate-fade-in ${isComingSoon ? 'opacity-60 hover:opacity-75' : ''}`}
              ...
              >
                ...
                <div className="flex items-start justify-between gap-2 mb-3">
                  <h3 className={`font-semibold text-foreground leading-snug ${isComingSoon ? '' : 'group-hover:text-primary'} transition-colors`}>
                    {guide.title}
                  </h3>
                  {isComingSoon && (
                    <Badge className="bg-amber-100 text-amber-800 border-amber-200 dark:bg-amber-900/30 dark:text-amber-400 dark:border-amber-800 shrink-0 text-[10px]">
                ...
                <p className="text-sm text-muted-foreground mb-4">
                  {isComingSoon
                    ? (language === 'bn' ? 'গাইড তৈরি হচ্ছে' : 'Guide in preparation')
                    : (
                      <>
                        {guide.step_count > 0 
