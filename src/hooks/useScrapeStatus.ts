...
type ScrapeStatusValue = 'success' | 'incomplete' | 'pending' | 'failed' | null;

interface ScrapeStatusMap {
  [url: string]: ScrapeStatusValue;
}
...
export async function fetchScrapeStats() {
  try {
    ...
    let success = 0, pending = 0, failed = 0, incomplete = 0;
    data?.forEach((item) => {
    ...
    return { success, pending, failed, incomplete };
  } catch {
    return { success: 0, pending: 0, failed: 0, incomplete: 0 };
  }
}
...
export function useScrapeStatus() {
  ...
  const [stats, setStats] = useState({ success: 0, pending: 0, failed: 0, incomplete: 0 });
  ...
  useEffect(() => {
    async function fetchStatuses() {
      ...
      try {
        ...
        data?.forEach((item) => {
          const status = item.scrape_status as ScrapeStatusValue;
          map[item.url] = status;
        ...
        setStatusMap(map);
        setStats({ success, pending, failed, incomplete });
      } catch (error) {
